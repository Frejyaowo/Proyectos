...
/* USER CODE BEGIN PV */
uint16_t estado = 0;//para la lectura para de flanco de subida a flanco de bajada
uint16_t uno[36];//toma el tiempo en ceros
uint16_t cero[36];//guarda los ceros y unos del control
uint16_t posicion =0;//posicion de los dos arreglos anteriores
uint16_t modo = 0;//para cambiar de modo run a set
uint16_t comandoP = 0;//posicion de los comandos en el arreglo
uint64_t comando[15];//guarda los comados ya decodificados
uint16_t tiempito[36];
uint64_t tiempoa;//tiempo finalizado,transcurrido
uint16_t tiempoe=0;//tiempo de flanco del boton del centro
char texto[20];
uint16_t modo_anterior = 0;
uint64_t recividoN = 0;
...
uint32_t customPow(uint16_t exponente);
void modo_set();
void modo_run();
				 ...
				 else if(estado == 1)
				 {
					 ...
					 for(int x=0 ; x<=70; x++)
					 {
						...
						else if(x % 2 == 0)//numero par
						{
							...
							{
								__HAL_TIM_SET_COUNTER(&htim2,0);
								//caso cuando este mostrando un cero
...
uint32_t customPow(uint16_t exponente) { //Para pasar de binario a decimal pero no se uso
	uint16_t base = 2;
	if (exponente == 0)
        return 1;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	if(GPIO_Pin == IR_Pin)//que interrupcion esta activada
	{
		switch(estado)
		{
		case 0://para inicializarr el tiempo de subida y guardar
			...
			tiempito[posicion] = x;
			//cero [posicion]=__HAL_TIM_GET_COUNTER(&htim2);
			tiempoa = tiempoa + __HAL_TIM_GET_COUNTER(&htim2);
		...
		case 1:
			//para inicializar el tiempo de bajada y guardar
